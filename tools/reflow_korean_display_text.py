...
REGEX_TEXT_FIXES: dict[str, tuple[tuple[str, str], ...]] = {
    'N0B00Y17-LOC.txt': (
        (
            r'_scholar_의 위치는\s+___scholardung_\s+음침한\s*(?:<ce>)?\s*구멍\s+속일 겁니다\.',
            '_scholar_는 ___scholardung_이라는 음침한 곳에 있을 겁니다.',
        ),
    ),
}


def apply_text_fixes(name: str, text: str) -> tuple[str, int]:
    count = 0
    for before, after in TEXT_FIXES.get(name, ()):
        occurrences = text.count(before)
        if occurrences:
            text = text.replace(before, after)
            count += occurrences
    for pattern, replacement in REGEX_TEXT_FIXES.get(name, ()):
        text, occurrences = re.subn(pattern, replacement, text)
        count += occurrences
    return text, count
...
def wrap_sentences(text: str, first_cap: int, later_cap: int | None = None) -> list[str]:
    ...
    for sentence in split_sentences(text):
        ...
        if (
            len(wrapped) == 2
            and len(wrapped[0].split()) == 1
            and display_width(sentence) <= sentence_cap + 2
        ):
            wrapped = [sentence]
        if wrapped:
...
def reflow_quest(path: Path, mirror: Path | None, write: bool) -> Change:
    ...
    # Apply explicit, screenshot-confirmed fixes before wrapping so width
    # calculations use the corrected phrase and repeated runs are stable.
    text, textual_fix_count = apply_text_fixes(path.name, text)
    ...
    out: list[str] = []
    i = 0
    change = Change(path=str(path), uppercase_g=upper_count, textual_fixes=textual_fix_count)
    while i < len(lines):
        line = lines[i]
        if i >= qbn_index:
            out.extend(lines[i:])
        ...
        if i > qrc_index and is_prose_line(line):
            label_re = re.compile(
                r'^(?:(?:유형|제작자|설명|수신|발신|제목|추신|품목 번호)\s*[:：]'
                r'|(?:Chapter\s+\S+|제\s*\d+장|장\s*\d+)\s*[:：]?)',
                re.I,
            )
